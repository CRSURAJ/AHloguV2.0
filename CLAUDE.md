...
- **JobsBoard** — split workspace: searchable job list on the left, create/edit form docked (sticky) in a ~480px right column; stacks form-first under 1020px. Job fields worker time-logging depends on: `jobId`, `assignedRoles`, `jobDocumentLinks`, plus an optional **Project** selector that sets `projectId` and, when the chosen project has sales orders, a **Sales order** selector that sets `salesOrderId`. Stats cards form the header card's lower half.

Project fields: `projectRef` (unique, case-insensitive — e.g. "AH-1088"), `customerName`, `location`, `department` (install/service), `value` (display string) + `valueAmount` (whole dollars, auto-parsed from `value` via `parseMoney` — "$186k"/"1.2M"/"186,000" all work; sums on BaajBoard use it), `projectType` + `controlPanel`, `salesOrders`, `stage`, `gates`, `trades`, `blocked`/`blockedReason`, `targetDate`, `deliveryDate`, `stageTargets`, `dateLog`, `activityLog`.
...
- **Project type & control panel** — `projectType` (`supply_loose` / `prefab` / `prefab_install` / `supply_loose_install`; `null` on legacy projects = everything applies) and the `controlPanel` flag (absent = in scope) scope the pipeline per project: `tradesForProject()` picks the applicable Build trades (plumbing+electrical on every type except plain Supply Loose; prefab only on Pre-Fab variants; controller only when a panel is in scope) and `criteriaForProject()` filters exit criteria via each criterion's `appliesWhen` (e.g. Engineering's drawing/BOM/approval criteria don't apply to Supply Loose; the controller-spec criterion needs a panel). Stored trade progress is never deleted — non-applicable trades are hidden and ignored by the Build gate. A stage left with zero applicable criteria auto-completes.
- **Sales orders** — `salesOrders` (`ProjectSalesOrder`: `id`, `soNumber`, `label`) records one SO per building/plant at Handover, managed in the drawer. Each SO should get a job by Build — jobs link back via `Job.salesOrderId`, and the drawer shows an advisory (not a gate) for SOs with no job; "New job" from an SO pre-fills the Jobs form.
...
- **Trades** (`TRADES`: plumbing/electrical/prefab/controller, filtered per project by `tradesForProject()`) appear in the Build stage — each has a checklist, a state machine (`not_started → in_progress → complete → signed_off`), and a block/reason flag. The Build "all trades complete" criterion is `auto` — it clears from trade state rather than a manual sign-off.
...
Persistence mirrors the jobs pattern: `useProjects` → `lib/projectStorage.ts` (localStorage cache + cloud) → `/projects` API → `PROJECTS_TABLE`. The Lambda sanitizes every project field server-side (allowlisted stage/department/projectType, bounded strings, structured `gates`/`trades`/`salesOrders`/`stageTargets`/`dateLog`/`activityLog`) and enforces unique `projectRef`.
