...
export default function WorkerStatusPanel({ onClose }: WorkerStatusPanelProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("focus", loadWorkerStatuses);

    return () => {
      window.clearInterval(intervalId);
      window.removeEventListener("focus", loadWorkerStatuses);
    };
  }, []);
  ...
  const summary = useMemo(() => {
    ...
    const pending = statuses.reduce(
      (total, item) => total + item.pendingSyncCount,
      0
    );

    return { working, onBreak, offline, pending };
  }, [statuses]);
